...
object CameraHelperSample {

  def apply() =

    val cameraHelperDiv = div(
      h1("CameraHelper Demo"),
      p(
        "Demonstrating the CameraHelper for visualizing camera frustum. The helper shows the camera's field of view, near and far planes, and target direction."
      ),
      div(
    ...
    // Create a second camera to visualize with the helper
    val helperCamera = new PerspectiveCamera(60, 1.5, 0.1, 50)
    helperCamera.position.set(3, 3, 3)
    helperCamera.lookAt(0, 0, 0)
    scene.add(helperCamera)

    // Create CameraHelper to visualize the helperCamera
    val cameraHelper = new CameraHelper(helperCamera)
    ...
    // Create some objects to demonstrate the camera visualization
    ...
    val animate: () => Unit = () => {
      ...
      // Animate the helper camera to show frustum changes
      helperCamera.position.x = js.Math.cos(time * 0.3) * 4
      helperCamera.position.z = js.Math.sin(time * 0.3) * 4
      helperCamera.position.y = js.Math.sin(time * 0.2) * 2 + 3
      helperCamera.lookAt(0, 0, 0)

      // Update the camera helper to reflect camera changes
      cameraHelper.update()
    ...
    cameraHelperDiv.ref.querySelector(".canvas-container").appendChild(renderer.domElement)
