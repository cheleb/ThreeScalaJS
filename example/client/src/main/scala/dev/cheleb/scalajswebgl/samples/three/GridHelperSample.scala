...
object GridHelperSample {

  def apply() =

    val gridHelperDiv = div(
      h1("GridHelper Demo"),
      p(
        "Demonstrating the GridHelper for visualizing a grid plane. The grid helps with spatial orientation and measurement in 3D space."
      ),
      div(
    ...
    // Create GridHelper with size 10 and 10 divisions
    val gridHelper = new GridHelper(10, 10)
    scene.add(gridHelper)

    // Create some objects to demonstrate the grid

    // 1. Cube positioned using the grid
    ...
    // 2. Sphere positioned using the grid
    ...
    // 3. Cone positioned using the grid
    ...
    // 4. Torus positioned using the grid
    ...
    gridHelperDiv.ref.querySelector(".canvas-container").appendChild(renderer.domElement)
